...
class PositionalEncoding3D(nn.Module):
    """
    3D Positional Encoding for feature space patches.
    Simple learnable absolute positional embeddings without physical assumptions.
    """
    def __init__(self, d_model, max_len=8, dropout=0.1):
        super(PositionalEncoding3D, self).__init__()
        self.d_model = d_model
        self.max_len = max_len
        
        # Simple learnable absolute positional embeddings
        # Let the model learn the optimal position representations for feature space
    ...
    def forward(self, x):
        """
        Args:
            x: (batch_size, seq_len, d_model) - flattened feature space patches
        Returns:
            x + simple positional encoding
        """
        seq_len = x.size(1)
        
        # Add simple absolute positional encoding
...
class MultiHeadAttention3D(nn.Module):
    ...
    def __init__(self, d_model, num_heads, dropout=0.1, max_len=8, use_relative_pos=True):
        ...
        # Relative position bias table for feature space patches
        if use_relative_pos:
        ...
        self.dropout = nn.Dropout(dropout)
        # Reduced scaling to strengthen attention patterns
        self.scale = math.sqrt(self.d_k) * 0.5
        
        # Learnable temperature for dynamic attention scaling
        # 🔥 OPTIMIZATION: Increase temperature to reduce activation saturation
        self.temperature = nn.Parameter(torch.ones(1) * 0.2)  # Increased from 0.1 to 0.2
    ...
    def forward(self, x):
        ...
        # Apply learnable temperature for dynamic attention strength
    ...
    def get_relative_position_stats(self):
        """Get statistics about relative position encoding."""
        ...
        if hasattr(self, 'last_relative_bias') and self.last_relative_bias is not None:
            ...
            stats['bias_contribution'] = 'inactive'  # Not currently active
...
class CrossAttention3D(nn.Module):
    ...
    def __init__(self, d_model, num_heads, dropout=0.1, enable_adsorbate_solvent_interaction=True):
        ...
        if enable_adsorbate_solvent_interaction:
            ...
            self.interaction_bias = nn.Parameter(torch.zeros(num_heads, 8, 8))  # 2x2x2 spatial positions
    ...
    def forward(self, query, key_value, feature_channels=None):
        ...
        if self.enable_adsorbate_solvent_interaction and hasattr(self, 'interaction_bias'):
            ...
            if scores.size(-1) == 8:  # 2x2x2 spatial arrangement
...
class TransformerEncoderLayer3D(nn.Module):
    ...
    def __init__(self, d_model, num_heads, d_ff, dropout=0.1, use_cross_attention=False):
        ...
        self.self_attention = MultiHeadAttention3D(d_model, num_heads, dropout, max_len=8, use_relative_pos=True)
        ...
        # 🔥 OPTIMIZATION: Use gentler activations to reduce saturation
        ...
        self.feed_forward = nn.Sequential(
            ...
            nn.Dropout(dropout * 1.2),  # Increased dropout for regularization
            nn.Linear(d_ff, d_model),
            nn.Dropout(dropout),
            nn.Tanh()  # Add output limiting to prevent extreme values
        )
        ...
        # 🔥 OPTIMIZATION: Reduce initial gate contribution to prevent early saturation
        self.gate_weight = nn.Parameter(torch.tensor(0.05))  # Reduced from 0.1 to 0.05
...
class VisionTransformer3D(nn.Module):
    ...
    def __init__(self, input_dim, d_model=256, num_heads=8, num_layers=2, dropout=0.1, use_cross_attention=True):
        ...
        # Enhanced input projection with residual connection for better feature preservation
        # 🔥 OPTIMIZATION: Reduce activation saturation (was 52.9% in logs)
        self.input_projection = nn.Sequential(
            nn.Linear(input_dim, d_model),
            nn.LayerNorm(d_model),
            nn.GELU(),  # Changed from SiLU to GELU - more gentle activation
            nn.Dropout(dropout * 0.7),  # Increased dropout to reduce overfitting
            nn.Linear(d_model, d_model),
            nn.Tanh()  # Added Tanh to limit output range to [-1,1], preventing saturation
        )
        ...
        # 3D Positional encoding for feature space patches
        self.pos_encoding = PositionalEncoding3D(d_model, max_len=8, dropout=dropout*0.5)
        
        # Optimized transformer encoder layers for molecular interaction modeling
        self.transformer_layers = nn.ModuleList([
            TransformerEncoderLayer3D(
                d_model, 
                num_heads, 
                d_model * 3,  # Larger FFN for better molecular interaction modeling
                dropout,
        ...
        # Enhanced global pooling with learnable molecular-aware weights
        # Adjust for H-bond and vdW interaction distances (~2-5 Å in 0.8Å voxels)
        self.pooling_weights = nn.Parameter(torch.tensor([0.4, 0.3, 0.3]))  # avg, max, distance-weighted
        
        # Molecular interaction enhancement layer
        # 🔥 OPTIMIZATION: Use gentler activation to prevent saturation
        self.interaction_enhancement = nn.Sequential(
    ...
    def forward(self, x, low_level_features=None, return_attention=False, feature_channels=None):
        ...
        # Apply molecular interaction enhancement
        x_enhanced = x + self.interaction_enhancement(x)
        
        # Enhanced global pooling for feature aggregation
        ...
        # Learnable weighted pooling (let model learn optimal combination)
        global_features = (self.pooling_weights[0] * avg_pooled + 
                          self.pooling_weights[1] * max_pooled +
                          self.pooling_weights[2] * avg_pooled)  # Third weight for stable training
    ...
    def get_attention_summary(self):
        ...
        for i, attention_map in enumerate(self.attention_maps):
            ...
            high_attention_ratio = (global_avg_attention > 0.8).float().mean().item()
            low_attention_ratio = (global_avg_attention < 0.1).float().mean().item()
            ...
            summary[f'layer_{i+1}'] = {
                ...
                'needs_temperature_increase': high_attention_ratio > 0.3,
                'needs_diversity_boost': head_diversity < 0.3,
                'attention_health': 'healthy' if 0.1 < high_attention_ratio < 0.3 and head_diversity > 0.3 else 'needs_attention'
            }
        
        # 🔥 NEW: Pooling weights analysis
        if hasattr(self, 'pooling_weights'):
            ...
            pooling_analysis = {
                ...
                'stability_weight': float(weights[2].item()),
                'avg_dominance': float((weights[0] + weights[2]).item()),  # Total avg pooling influence
                'max_dominance': float(weights[1].item()),
                'pooling_balance': float((weights[0] + weights[2]) / weights[1].item()),
                'learned_strategy': 'avg_dominant' if (weights[0] + weights[2]) > weights[1] * 1.5 else 'balanced' if abs((weights[0] + weights[2]) - weights[1]) < 0.2 else 'max_dominant'
            }
...
class AttentionCNNTransformer_2_2(nn.Module):
    """
    ...
    def __init__(self, in_channels=28,  # Changed default from 15 to 28 for type_2 format
                 ...
                 transformer_heads=8,  # Kept good head diversity
                 transformer_layers=2,
                 feature_names=None):  # Add feature_names parameter
        
        super(AttentionCNNTransformer_2_2, self).__init__()
        ...
        # 🔥 OPTIMIZATION: Enhance adsorbate feature extraction (current importance only 17.1%)
        ...
        self.adsorbate_processor = nn.Sequential(
            ...
            nn.BatchNorm3d(16),
            nn.SiLU(inplace=True),  # SiLU for better molecular modeling
            ...
            nn.BatchNorm3d(20),
            nn.ReLU(inplace=True),  # Use ReLU to maintain sparse nature but enhance active features
        ...
        self.layer1 = ResidualBlock3D(32, 48, stride=1, use_cbam=True, use_self_attention=False)
        self.pool1 = nn.AvgPool3d(2)  
        
        self.layer2 = ResidualBlock3D(48, 64, stride=1, use_cbam=True, use_self_attention=True)
        self.pool2 = nn.AvgPool3d(2)  
        
        self.layer3 = ResidualBlock3D(64, 80, stride=1, use_cbam=True, use_self_attention=True) 
        
        # Final feature layer
        self.layer4 = ResidualBlock3D(80, 96, stride=1, use_cbam=True, use_self_attention=False)
        ...
        if self.use_transformer:
            ...
            classifier_input_dim = 96 * 2 * 2 * 2  # Updated flattened features = 768
        ...
        if self.use_transformer:
            ...
            self.classifier = nn.Sequential(
                nn.Flatten(),
                nn.Linear(classifier_input_dim, 256),  # Increased capacity due to larger feature space
                nn.BatchNorm1d(256),
                nn.LeakyReLU(negative_slope=0.01, inplace=True),
                nn.Dropout(dropout_rate),
                
                nn.Linear(256, 96),             # Intermediate layer
                nn.BatchNorm1d(96),
                nn.LeakyReLU(negative_slope=0.01, inplace=True),
                nn.Dropout(dropout_rate * 0.7),
                
                nn.Linear(96, 24),              # Final hidden layer
                nn.BatchNorm1d(24),
                nn.LeakyReLU(negative_slope=0.01, inplace=True),
                nn.Dropout(dropout_rate * 0.5),
                
                nn.Linear(24, 1)               # Output layer
            )
