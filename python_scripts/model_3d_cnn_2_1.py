...
class VisionTransformer3D(nn.Module):
    ...
    def forward(self, x, low_level_features=None, return_attention=False, feature_channels=None):
        ...
        # Enhanced physics-aware global pooling for molecular interactions
        ...
        # Molecular distance-weighted pooling for H-bond and vdW interactions
        seq_len = x_enhanced.size(1)
        if seq_len == 8:  # 2x2x2 grid case
            # Create molecular interaction distance weights
            # In 0.8Å voxels, H-bonds (~2-3Å) and vdW (~3-5Å) are within 3-6 voxel distances
            positions_3d = [(i%2, (i//2)%2, i//4) for i in range(seq_len)]
            center_3d = (0.5, 0.5, 0.5)  # Adsorbate center
            
            distance_weights = []
            for pos in positions_3d:
                dist = ((pos[0] - center_3d[0])**2 + (pos[1] - center_3d[1])**2 + (pos[2] - center_3d[2])**2)**0.5
                # Molecular interaction weighting: exponential decay with distance
                # Optimized for H-bond (strong, short) and vdW (moderate, medium) interactions
                weight = torch.exp(torch.tensor(-dist * 1.5))  # Adjusted for molecular interactions
                distance_weights.append(weight)
            
            distance_weights = torch.tensor(distance_weights, device=x_enhanced.device)
            distance_weights = distance_weights / distance_weights.sum()  # Normalize
            
            # Apply molecular distance-weighted pooling
            weighted_pooled = torch.sum(x_enhanced * distance_weights.unsqueeze(0).unsqueeze(2), dim=1)
            
            # Learnable combination optimized for molecular interactions
...
class AttentionCNNTransformer_2_1(nn.Module):
    """
    ...
    def __init__(self, in_channels=28,  # Changed default from 15 to 28 for type_2 format
        ...
        super(AttentionCNNTransformer_2_1, self).__init__()
        ...
        # Adsorbate occupies central small volume, mostly zeros, so use minimal processing
        self.adsorbate_processor = nn.Sequential(
            ...
            nn.Conv3d(16, 24, kernel_size=3, padding=1, bias=False),  # Reduced from 32 to 24, moderate expansion
            nn.BatchNorm3d(24),
            nn.SiLU(inplace=True),
            nn.Dropout3d(0.05)
        )
    ...
    def _get_branch_recommendations(self):
        ...
        if self.branch_statistics['magnitude_ratio'] == 0.0:
        ...
        if self.branch_statistics['sparsity_ratio'] > 5.0:
        ...
        magnitude_ratio = self.branch_statistics['magnitude_ratio']
        ...
        if self.branch_statistics['interaction_strength'] < 0.01:
        ...
        if 'capacity_ratio' in self.branch_statistics:
            capacity_ratio = self.branch_statistics['capacity_ratio']
