...
store_grids_pickle.py
    The purpose of this code is to generate voxel grids for the entire dataset,
...
def generate_and_store_adsorbate_grids(
    ...
    """
    Generate voxel grids for one complete adsorbate, apply augmentation, and store as pickle.
    ...
    OUTPUTS:
        adsorbate_data: dict, processed data structure containing all grids
    ...
    for snapshot_index in range(snapshot_start, snapshot_end):
        ...
        # Set metadata from first snapshot
        if snapshot_index == snapshot_start:
            adsorbate_data['metadata']['grid_shape'] = generate_voxel_grids.voxel_grid.shape
            adsorbate_data['metadata']['feature_channel_mapping'] = getattr(generate_voxel_grids, 'feature_channel_mapping', None)
        ...
        # Store snapshot data (remove voxel_grid_for_plots to save memory)
        # Create augmented_data without voxel_grid_for_plots
        augmented_data_for_storage = []
        for item in augmented_data:
            storage_item = {
                'voxel_grid': item['voxel_grid'],  # Only keep ML training grid
...
def check_all_pickle_files_exist(
    ...
    # Calculate grid dimensions and shape string
