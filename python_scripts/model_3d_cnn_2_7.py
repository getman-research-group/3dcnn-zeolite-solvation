...
Key optimizations in v2_7 (Plan A):
...
class AttentionCNN_2_7(nn.Module):
    """
    ...
    - Removed attention: layer2 (48→64), layer3 (64→80) for maximum efficiency - ❌ TOO LATE
    ...
    def __init__(self, in_channels=28,  # Default 28 for type_2 format
        ...
        super(AttentionCNN_2_7, self).__init__()
        ...
        self.interaction_attention = CBAM3D(
            in_channels=72,           # Process complete 72 channels (24 adsorbate + 48 solvent)
            kernel_size=5,            # Maintain larger kernel for H-bond capture (2-3Å at 0.8Å resolution)
            dropout=0.08, 
            enable_group_interaction=True, 
            group_split=[24, 48]      # 🔑 CRITICAL: Correct group split (24 adsorbate + 48 solvent)
        )
        
        # Interaction fusion layer: Pure feature mixing and dimensionality reduction (no attention)
        ...
        # 🏗️ ENHANCED CNN BACKBONE: Deeper feature extraction, shallower regressor
        # Add layer3 for better feature learning while keeping total parameters similar
        self.layer1 = ResidualBlock3D(32, 48, stride=1, use_cbam=True)
        self.pool1 = nn.AvgPool3d(2)  # 20×20×20 → 10×10×10
        
        self.layer2 = ResidualBlock3D(48, 64, stride=1, use_cbam=False)
        self.pool2 = nn.AvgPool3d(2)  # 10×10×10 → 5×5×5
        
        # 🔥 NEW: Add layer3 for deeper feature extraction
        self.layer3 = ResidualBlock3D(64, 80, stride=1, use_cbam=False)  # More balanced channel expansion
        ...
        # 🎯 DRAMATICALLY SIMPLIFIED MLP: Leverage richer CNN features
        regressor_input_dim = 80 * 2 * 2 * 2  # 640 (balanced between feature richness and parameter efficiency)

        # Streamlined regressor: fewer parameters but richer input features
        self.regressor = nn.Sequential(
            nn.Flatten(),
            
            # 🔥 SIMPLIFIED: Direct path with stronger regularization
            nn.Linear(regressor_input_dim, 128),  # 768 → 128 (6x reduction)
            nn.BatchNorm1d(128),
            nn.ReLU(inplace=True),
            nn.Dropout(dropout_rate * 0.9),  # High dropout since CNN does heavy lifting
            
            # Direct output - let CNN features drive prediction
    ...
    def forward(self, x, monitor_activations=False, detailed_analysis=None, feature_channels=None):
        ...
        # This preserves the physical meaning of channel groups (24 adsorbate + 48 solvent)
        attended_features = self.interaction_attention(combined_features)  # (batch, 72, 20, 20, 20)
        if monitor_activations:
        ...
        interaction_features = self.interaction_conv(attended_features)   # (batch, 32, 20, 20, 20)
        if monitor_activations:
        ...
        # 🔥 NEW: Layer3 for deeper feature extraction
