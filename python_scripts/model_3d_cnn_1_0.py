"""
model_3d_cnn_transformer.py
This file contains the definitions for the 3D-CNN model with CBAM attention mechanism 
enhanced with Vision Transformer (ViT) for global feature modeling.
...
You can import the final model `AttentionCNNTransformer` into your main training script.
"""
...
class CrossAttention3D(nn.Module):
    """
    Cross-Attention layer for fusing CNN multi-level features.
    Query from high-level features, Key/Value from low-level features.
    """
    def __init__(self, d_model, num_heads, dropout=0.1):
        ...
        self.w_q = nn.Linear(d_model, d_model, bias=False)
        self.w_k = nn.Linear(d_model, d_model, bias=False)  # For low-level features
        self.w_v = nn.Linear(d_model, d_model, bias=False)  # For low-level features
        self.w_o = nn.Linear(d_model, d_model)
        ...
        # Store attention weights for monitoring and analysis
        self.last_attention_weights = None
    ...
    def forward(self, query, key_value):
        ...
        # Store attention weights and calculate statistics for monitoring
        self.last_attention_weights = attention_weights.detach()
        
        # Calculate cross-attention monitoring statistics
        with torch.no_grad():
            # Cross-fusion strength: how much low-level features are utilized
            ...
            # Low-level utilization: variance in attention across low-level features
            low_level_utilization = attention_weights.std().item()
            
            # Attention focus: maximum attention weight (concentration)
...
class TransformerEncoderLayer3D(nn.Module):
    ...
    def forward(self, x, low_level_features=None):
        """
        Args:
        ...
        # Cross-attention for multi-level feature fusion
        if self.use_cross_attention and low_level_features is not None:
            norm_x_cross = self.norm_cross(x)
            cross_output = self.cross_attention(norm_x_cross, low_level_features)
...
class VisionTransformer3D(nn.Module):
    ...
    def __init__(self, input_dim, d_model=256, num_heads=8, num_layers=2, dropout=0.1, use_cross_attention=True):
        ...
        self.transformer_layers = nn.ModuleList([
            TransformerEncoderLayer3D(
                ...
                d_model*2, 
                dropout,
                use_cross_attention=(use_cross_attention and i > 0)  # Only use cross-attention after first layer
            )
    ...
    def forward(self, x, low_level_features=None, return_attention=False):
        """
        ...
        for i, layer in enumerate(self.transformer_layers):
            if i == 0 or not self.use_cross_attention or low_level_projected is None:
                ...
                x = layer(x, low_level_projected)
            
            # Store attention weights for analysis
            if hasattr(layer.self_attention, 'last_attention_weights'):
                self.attention_maps.append(layer.self_attention.last_attention_weights.clone())
            
            # Store cross-attention weights if available
            if hasattr(layer, 'cross_attention') and hasattr(layer.cross_attention, 'last_attention_weights'):
...
class LightSelfAttention3D(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            head_differences = torch.std(attn.mean(dim=(0, -1)), dim=0).mean().item()
...
class AttentionCNNTransformer(nn.Module):
    """
    ...
    def __init__(self, in_channels=15,
        ...
        super(AttentionCNNTransformer, self).__init__()
        ...
        # Parameter-optimized configuration for ~1.5M total parameters
        # Focus on efficiency while maintaining molecular interaction modeling capability
    ...
    def forward(self, x, monitor_activations=False, detailed_analysis=None):
        """
        Enhanced forward pass with comprehensive monitoring
        
        Args:
            x: Input voxel grid (batch_size, 15, 20, 20, 20)
        ...
        if self.use_transformer:
            ...
            x_before_transformer = x.clone() if detailed_analysis else None
            
            # Select appropriate low-level features for cross-attention
            # Use layer2 features as they provide good balance of local and global info
            low_level_features = layer2_features
            
            if detailed_analysis:
                # Get detailed transformer analysis with cross-attention
                x, transformer_attention_maps, cross_attention_maps = self.vision_transformer(
                    x, low_level_features=low_level_features, return_attention=True
                )
                monitoring_data['attention_info']['vision_transformer_detailed'] = self.analyze_transformer_attention_detailed(transformer_attention_maps)
                
                # 🎯 New: Monitor cross-attention effectiveness
                if cross_attention_maps:
                    monitoring_data['attention_info']['cross_attention_maps'] = cross_attention_maps
                    monitoring_data['attention_info']['cross_attention_analysis'] = self.analyze_cross_attention_effectiveness(cross_attention_maps, low_level_features, x)
                
                # Monitor multi-level feature fusion effectiveness
                monitoring_data['training_insights']['transformer_vs_cnn_contribution'] = self.compare_transformer_cnn_contribution(x_before_transformer, x)
                monitoring_data['training_insights']['cross_attention_fusion_strength'] = self.evaluate_cross_attention_fusion(low_level_features, x)
            else:
                x = self.vision_transformer(x, low_level_features=low_level_features)  # (batch, transformer_dim)
                
                # Monitor basic cross-attention statistics even without detailed analysis
                if monitor_activations:
                    # Try to get basic cross-attention stats
                    try:
                        basic_cross_stats = self.get_cross_attention_basic_stats()
                        if basic_cross_stats:
                            monitoring_data['attention_info']['cross_attention_basic'] = basic_cross_stats
                    except:
    ...
    def analyze_feature_utilization(self, x):
        """Analyze how each of the 15 input features is being utilized"""
        if x.dim() != 5 or x.size(1) != 15:
            return {}
        ...
        for i, feature_name in enumerate(feature_names):
            ...
            feature_stats[f'feature_{i:02d}_{feature_name}'] = {
                ...
                'dynamic_range': float(np.max(feature_data) - np.min(feature_data)) if np.std(feature_data) > 1e-6 else 0.0
            }
    ...
    def analyze_input_features(self, x):
        """🔬 Comprehensive input feature analysis for model optimization"""
        if x.dim() != 5 or x.size(1) != 15:
            return {}
        ...
        batch_flattened = x.view(x.size(0), x.size(1), -1).mean(dim=2)  # (batch, 15)
    ...
    def get_attention_summary(self):
        ...
        for layer_name, attention_data in latest_attention.items():
            ...
            if 'self_attention_strength' in attention_data:
                layer_summary['self_attention'] = f"strength={attention_data['self_attention_strength']:.3f}"
            
            if 'attention_entropy' in attention_data:
                layer_summary['entropy'] = f"{attention_data['attention_entropy']:.2f}"
            
            # Handle Vision Transformer layer data
            if 'transformer_layer_1' in attention_data:
                tr_data = attention_data['transformer_layer_1']
                if 'global_attention_strength' in tr_data:
                    layer_summary['transformer'] = f"global_strength={tr_data['global_attention_strength']:.3f}"
            
            # Handle Vision Transformer summary data
            if 'layer_1' in attention_data:
                vt_data = attention_data['layer_1']
                if 'mean_attention' in vt_data:
                    layer_summary['vision_transformer'] = f"mean={vt_data['mean_attention']:.3f}, entropy={vt_data['attention_entropy']:.2f}"
    ...
    # 🎯 NEW MONITORING METHODS FOR CROSS-ATTENTION AND SELF-ATTENTION
    ...
    def analyze_cross_attention_effectiveness(self, cross_attention_maps=None, low_level_features=None, output_features=None):
        """Analyze the effectiveness of cross-attention fusion"""
        # If specific data is provided, analyze it directly
        if cross_attention_maps is not None:
            analysis = {
                'fusion_layers_count': len(cross_attention_maps) if cross_attention_maps else 0,
            ...
            if cross_attention_maps:
                # Analyze provided cross-attention maps
                fusion_strengths = []
                for layer_idx, attention_map in enumerate(cross_attention_maps):
                    if attention_map is not None:
                        # Calculate fusion strength from attention weights
                        avg_attention = attention_map.mean().item()
                        fusion_strengths.append(avg_attention)
                
                if fusion_strengths:
                    analysis['average_fusion_strength'] = sum(fusion_strengths) / len(fusion_strengths)
                    analysis['fusion_diversity'] = float(torch.tensor(fusion_strengths).std().item()) if len(fusion_strengths) > 1 else 0.0
                
                # Calculate feature enhancement if both input and output features are provided
                if low_level_features is not None and output_features is not None:
                    input_magnitude = torch.mean(torch.abs(low_level_features)).item()
                    output_magnitude = torch.mean(torch.abs(output_features)).item()
                    analysis['feature_enhancement_ratio'] = output_magnitude / (input_magnitude + 1e-8)
            
            return analysis
        
        # Fallback: analyze from stored data
        if not hasattr(self, '_cross_attention_data') or not self._cross_attention_data:
            return {'status': 'No cross-attention data available'}
        
        analysis = {
            'fusion_layers_count': len(self._cross_attention_data),
            'average_fusion_strength': 0.0,
            'fusion_diversity': 0.0,
            'feature_enhancement_ratio': 0.0
        }
        
        # Calculate average fusion strength across layers
        fusion_strengths = []
        for layer_name, cross_data in self._cross_attention_data.items():
            if 'cross_fusion_strength' in cross_data and isinstance(cross_data['cross_fusion_strength'], (int, float)):
                fusion_strengths.append(cross_data['cross_fusion_strength'])
        
        if fusion_strengths:
            analysis['average_fusion_strength'] = float(np.mean(fusion_strengths))
            analysis['fusion_diversity'] = float(np.std(fusion_strengths))
        
        # Add entropy and information preservation metrics
        entropy_values = []
        for layer_name, cross_data in self._cross_attention_data.items():
            if 'attention_entropy' in cross_data and isinstance(cross_data['attention_entropy'], (int, float)):
                entropy_values.append(cross_data['attention_entropy'])
        
        if entropy_values:
            analysis['average_attention_entropy'] = float(np.mean(entropy_values))
            analysis['entropy_consistency'] = float(np.std(entropy_values))
        
        return analysis
        
        # Measure feature enhancement (how much cross-attention changes the features)
        if low_level_features is not None and high_level_output is not None:
            # Simple proxy: compare feature magnitudes
            low_level_magnitude = torch.mean(torch.abs(low_level_features)).item()
            high_level_magnitude = torch.mean(torch.abs(high_level_output)).item()
            analysis['feature_enhancement_ratio'] = float(high_level_magnitude / (low_level_magnitude + 1e-8))
        
        return analysis
    
    def evaluate_cross_attention_fusion(self, low_level_features=None, high_level_output=None):
        """Evaluate the quality of cross-attention feature fusion"""
        # If specific features are provided, analyze them directly
        if low_level_features is not None and high_level_output is not None:
            evaluation = {}
            ...
            evaluation['information_preservation'] = high_level_magnitude / (low_level_magnitude + 1e-8)
            evaluation['fusion_efficiency'] = min(1.0, evaluation['information_preservation'] / 2.0)  # Normalize to [0,1]
            
            # Calculate feature correlation if both have same spatial structure
            if low_level_features.shape[0] == high_level_output.shape[0]:  # Same batch size
                try:
                    # Flatten spatial dimensions for correlation
                    low_flat = low_level_features.view(low_level_features.size(0), -1).mean(dim=1)
                    high_flat = high_level_output.view(high_level_output.size(0), -1).mean(dim=1)
                    
                    if low_flat.size(0) > 1:  # Need at least 2 samples for correlation
                        correlation = torch.corrcoef(torch.stack([low_flat, high_flat]))[0, 1]
                        evaluation['feature_correlation'] = float(correlation.item()) if not torch.isnan(correlation) else 0.0
                    else:
                        evaluation['feature_correlation'] = 0.0
                except:
                    evaluation['feature_correlation'] = 0.0
            
            return evaluation
        
        # Fallback: evaluate from stored data
        if not hasattr(self, '_cross_attention_data') or not self._cross_attention_data:
            return {'status': 'No cross-attention data for fusion evaluation'}
        
        evaluation = {}
        
        # Collect fusion quality metrics from stored data
        preservation_scores = []
        utilization_scores = []
        
        for layer_name, cross_data in self._cross_attention_data.items():
            if 'information_preservation' in cross_data and isinstance(cross_data['information_preservation'], (int, float)):
                preservation_scores.append(cross_data['information_preservation'])
            if 'low_level_utilization' in cross_data and isinstance(cross_data['low_level_utilization'], (int, float)):
                utilization_scores.append(cross_data['low_level_utilization'])
        
        if preservation_scores:
            evaluation['average_information_preservation'] = float(np.mean(preservation_scores))
            evaluation['preservation_consistency'] = float(np.std(preservation_scores))
        
        if utilization_scores:
            evaluation['average_low_level_utilization'] = float(np.mean(utilization_scores))
            evaluation['utilization_consistency'] = float(np.std(utilization_scores))
        
        # Overall fusion quality assessment
        if preservation_scores and utilization_scores:
            overall_quality = (np.mean(preservation_scores) + np.mean(utilization_scores)) / 2.0
            evaluation['overall_fusion_quality'] = float(overall_quality)
            
            # Quality assessment
            if overall_quality > 0.8:
                evaluation['fusion_assessment'] = 'excellent'
            elif overall_quality > 0.6:
                evaluation['fusion_assessment'] = 'good'
            elif overall_quality > 0.4:
                evaluation['fusion_assessment'] = 'moderate'
            else:
                evaluation['fusion_assessment'] = 'poor'
        
        return evaluation
    
    def get_cross_attention_basic_stats(self):
        """Get basic cross-attention statistics for monitoring"""
        if not hasattr(self.vision_transformer, 'cross_attention_maps') or not self.vision_transformer.cross_attention_maps:
            return {'status': 'No cross-attention data'}
        
        stats = {
            'num_cross_attention_layers': len(self.vision_transformer.cross_attention_maps),
            'fusion_strength_distribution': []
        }
        
        for i, attention_map in enumerate(self.vision_transformer.cross_attention_maps):
            layer_stats = {
                'layer_index': i,
                'mean_attention': float(attention_map.mean().item()),
                'max_attention': float(attention_map.max().item()),
                'attention_sparsity': float((attention_map < 0.1).float().mean().item())
            }
            stats['fusion_strength_distribution'].append(layer_stats)
...
if __name__ == '__main__':
    ...
    # Define batch_size here so it's available for both sections
    batch_size = 16  # Reduced for transformer testing
    
    # Test both architectures
    print("\n1️⃣  Testing CNN-only architecture:")
    print("-" * 40)
    
    # 1. Instantiate the CNN-only model
    model_cnn = AttentionCNNTransformer(in_channels=15, use_transformer=False)
    
    # Test CNN-only model
    try:
        from torchinfo import summary
        input_shape = (batch_size, 15, 20, 20, 20)
        print("CNN-only Model Summary:")
        summary(model_cnn, input_size=input_shape, col_names=["input_size", "output_size", "num_params"])
    except ImportError:
        print("torchinfo not installed. Showing basic info:")
        print(f"CNN-only model: {model_cnn}")
    
    # Test forward pass
    dummy_input = torch.randn(batch_size, 15, 20, 20, 20)
    output_cnn = model_cnn(dummy_input)
    print(f"CNN Input shape: {dummy_input.shape}")
    print(f"CNN Output shape: {output_cnn.shape}")
    
    # Model analysis
    model_cnn.print_model_analysis()
    
    print("\n" + "="*60)
    print("2️⃣  Testing CNN-Transformer hybrid architecture:")
    print("-" * 40)
    
    # 2. Instantiate the hybrid CNN-Transformer model
    model_hybrid = AttentionCNNTransformer(
        in_channels=15, 
        use_transformer=True,
        transformer_dim=256,
        transformer_heads=8,
    ...
    try:
        from torchinfo import summary
        print("Hybrid CNN-Transformer Model Summary:")
        summary(model_hybrid, input_size=input_shape, col_names=["input_size", "output_size", "num_params"])
    except ImportError:
        print("torchinfo not installed. Showing basic info:")
        print(f"Hybrid model: {model_hybrid}")
    
    # Test forward pass
    output_hybrid = model_hybrid(dummy_input)
    print(f"Hybrid Input shape: {dummy_input.shape}")
    print(f"Hybrid Output shape: {output_hybrid.shape}")
    
    # Model analysis
    model_hybrid.print_model_analysis()
    
    # Test transformer attention analysis
    if model_hybrid.use_transformer:
        print("\n🧠 Testing Transformer Attention Analysis:")
        # Run a forward pass to collect attention
        _ = model_hybrid(dummy_input)
        attention_analysis = model_hybrid.get_transformer_attention_analysis()
        if isinstance(attention_analysis, dict):
            print("✅ Transformer attention analysis successful!")
            for layer, data in attention_analysis.items():
                if 'layer_summary' in data:
                    summary = data['layer_summary']
                    print(f"  {layer}: center_focus={summary['avg_center_focus']:.3f}, "
                          f"spread={summary['avg_attention_spread']:.3f}, "
                          f"diversity={summary['head_diversity']:.3f}")
        else:
            print(f"⚠️  Attention analysis: {attention_analysis}")
    
    print("\n" + "="*60)
    print("3️⃣  Comparing Model Sizes:")
    print("-" * 40)
    
    cnn_params = sum(p.numel() for p in model_cnn.parameters())
    hybrid_params = sum(p.numel() for p in model_hybrid.parameters())
    
    print(f"CNN-only parameters: {cnn_params:,}")
    print(f"Hybrid parameters: {hybrid_params:,}")
    print(f"Parameter increase: {hybrid_params - cnn_params:,} ({(hybrid_params/cnn_params - 1)*100:.1f}%)")
