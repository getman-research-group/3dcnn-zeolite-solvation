...
class CNN3DTrainer:
    ...
    def print_fold_completion_analysis(self, fold_idx, train_metrics, test_metrics, 
        ...
        print(f"\n{'='*80}")
        print(f"📋 FOLD {fold_idx + 1}/{self.n_splits} EPOCH HISTORY (COPY-READY)")
        print(f"{'='*80}")
    ...
    def train_single_model(self, X_train, y_train, X_test, y_test, fold_idx, train_idx, test_idx):
        ...
        # 🎯 STABILITY-FOCUSED LR scheduling - conservative to minimize sudden changes
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            optimizer,
            mode='min',
            factor=0.7,       # Gentler reduction to avoid sudden jumps
            patience=6,       # Longer patience to avoid premature adjustments
            min_lr=1e-7,      # Lower minimum for fine-tuning
            threshold=1e-5,   # Smaller threshold for more sensitivity
            cooldown=2,       # Prevent frequent adjustments
            verbose=True
        )
        
        # 🛑 STABLE early stopping - consistent fold performance
        best_test_loss = float('inf')
        best_model_state = None
        patience_counter = 0
        
        # 🛑 STABILITY-FOCUSED early stopping - track test loss stability
        best_test_loss = float('inf')
        best_model_state = None
        patience_counter = 0
        
        # 🔥 NEW: Stability tracking to reduce test loss volatility
        recent_test_losses = []  # Track recent test losses for stability analysis
        stability_window = 5     # Window size for stability calculation
        stable_loss_count = 0    # Count of stable epochs
        
        # Early stopping - optimized to detect stable performance early
        early_stop_patience = 15  # Increased patience for stability
        
        # For test mode, allow minimal patience
        if self.test_mode:
            early_stop_patience = 1
        
        # Allow sufficient initial learning - reduced since lr=0.0002 will be slower
        min_epochs = 8  # Reduced min epochs for lr=0.0002
        ...
        if self.verbose:
            print(f"    🎯 STABILITY-FOCUSED Training Strategy (optimized for test loss stability):")
            print(f"      Learning rate: {self.learning_rate:.6f} (direct input, no adjustment)")
            print(f"      Weight decay: {weight_decay:.0e} (reduced for stable training)")
            print(f"      Dropout rate: {dropout_rate:.2f} (minimized to reduce volatility)")
            print(f"      Early stop patience: {early_stop_patience} (increased for stability)")
            print(f"      LR scheduler: factor=0.7, patience=6, cooldown=2 (conservative)")
            print(f"      Gradient clipping: {max_grad_norm} (stronger for stability)")
            print(f"      Overfitting threshold: 3.0 (higher, less sensitive)")
            print(f"    🔧 VOLATILITY REDUCTION FEATURES:")
            print(f"      • Dramatically reduced dropout rates")
            print(f"      • Stronger gradient clipping (0.5 vs 1.0)")
            print(f"      • Test loss stability tracking")  
            print(f"      • Stability-based early stopping")
            print(f"      • Conservative learning rate scheduling")
        ...
        for epoch in range(self.epochs):
            ...
            # Enhanced early stopping with STABILITY-FOCUSED approach
            improvement_threshold = 1e-5  # Smaller threshold for more sensitive improvement detection
            
            # 🔥 NEW: Add current test loss to stability tracking
            recent_test_losses.append(test_loss)
            if len(recent_test_losses) > stability_window:
                recent_test_losses.pop(0)
            
            # Calculate test loss stability score
            test_loss_stability = 0
            if len(recent_test_losses) >= stability_window:
                test_loss_std = np.std(recent_test_losses)
                test_loss_mean = np.mean(recent_test_losses)
                test_loss_cv = test_loss_std / test_loss_mean if test_loss_mean > 0 else 1.0
                test_loss_stability = max(0, 1 - test_loss_cv * 5)  # Stability score (0-1)
            
            # Enhanced model saving logic with stability consideration
            is_significant_improvement = test_loss < best_test_loss * 0.99  # At least 1% improvement
            is_any_improvement = test_loss < best_test_loss - improvement_threshold
            is_stable = test_loss_stability > 0.7  # Good stability
            
            if is_significant_improvement:
                # Significant improvement - always save
                best_test_loss = test_loss
                best_model_state = model.state_dict().copy()
                patience_counter = 0
                improvement_flag = "✓"
                stable_loss_count = 0
            elif is_any_improvement and (epoch < min_epochs or is_stable):
                # Small improvement but stable or still in early training
                best_test_loss = test_loss
                best_model_state = model.state_dict().copy()
                patience_counter = 0
                improvement_flag = "✓STABLE" if is_stable else "✓"
                stable_loss_count = stable_loss_count + 1 if is_stable else 0
            else:
                patience_counter += 1
                improvement_flag = ""
                if is_stable:
            ...
            # 🎯 STABILITY-FOCUSED overfitting and early stopping
            # More conservative overfitting detection to reduce premature stopping
            severe_overfitting_threshold = 3.0  # Higher threshold for more stability
            
            # Check for severe overfitting
            if overfitting_ratio > severe_overfitting_threshold and patience_counter >= 5:
                if self.verbose:
                    print(f"\n{'─'*80}")
                    print(f"🛑 EARLY TERMINATION - SEVERE OVERFITTING DETECTED")
                    print(f"{'─'*80}")
                    print(f"  Stopping at epoch {epoch + 1} due to severe overfitting (ratio: {overfitting_ratio:.2f})")
                    print(f"  Best test loss was: {best_test_loss:.4f}")
                break
            
            # 🔥 NEW: Stability-based early stopping
            # If we have stable performance for several epochs, consider early stopping
            if (epoch >= min_epochs and 
                len(recent_test_losses) >= stability_window and 
                test_loss_stability > 0.8 and 
                test_loss < best_test_loss * 1.02 and  # Within 2% of best
                stable_loss_count >= 4):  # Stable for at least 4 epochs
                
                if self.verbose:
                    print(f"\n{'─'*80}")
                    print(f"✅ STABILITY EARLY STOP - STABLE PERFORMANCE ACHIEVED")
                    print(f"{'─'*80}")
                    print(f"  Stopping at epoch {epoch + 1} due to stable performance")
                    print(f"  Stability score: {test_loss_stability:.3f}")
                    print(f"  Test loss CV: {np.std(recent_test_losses)/np.mean(recent_test_losses):.4f}")
                    print(f"  Stable epochs count: {stable_loss_count}")
                break
            ...
            if self.verbose:
                ...
                # Format stability information
                stability_info = ""
                if test_loss_stability > 0:
                    stability_info = f", Stability={test_loss_stability:.2f}"
                    if stable_loss_count > 0:
                        stability_info += f"({stable_loss_count})"
                ...
                epoch_log_line = (f"  Epoch {epoch + 1:3d}/{self.epochs}: "
                                ...
                                f"LR={old_lr:.6f}{lr_change}{grad_info}{stability_info}, "
        ...
        # Load best model state - STABILITY-FOCUSED approach
        if best_model_state is not None:
            model.load_state_dict(best_model_state)
            if self.verbose:
                print(f"    ✅ Loaded best model (test loss: {best_test_loss:.4f})")
                if test_loss_stability > 0:
                    print(f"    📊 Final stability score: {test_loss_stability:.3f}")
        else:
            if self.verbose:
...
def parse_arguments():
    ...
    # Output file prefix argument - hardcoded for train_3d_cnn_2_7.py
    parser.add_argument('--output-prefix', type=str, default='model_2_7',
