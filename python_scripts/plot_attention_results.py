...
class SpatialAttentionAnalyzer:
    ...
    def plot_comprehensive_attention_overlay_simple(self,
                                                     zeolite,
                                                     env,
                                                     pore_type,
                                                     adsorbate,
                                                     ...
                                                     save_fig=False):
        """
        Simplified attention visualization - single plane view (YZ, XZ, or XY)
        
        Layout:
        - Row 1: Specified plane attention heatmaps for all 3 CBAM modules (horizontal)
        - Row 2: Radial distribution plots for all 3 modules (horizontal)
        ...
        print(f"\n🎨 SIMPLIFIED ATTENTION VISUALIZATION ({view_angle} Plane)")
        print("="*80)
        print(f"Sample: {zeolite} | {env} | {pore_type} | {adsorbate} | snapshot {snapshot} | voxel {voxel_id}")
        ...
        # Layout: 2 rows × 3 columns (one column per module)
        # height_ratios: Make row 2 (RDF plots) shorter than row 1 (heatmaps)
        fig = plt.figure(figsize=(18, 11))
        gs = fig.add_gridspec(2, 3,
                              hspace=0.04, # Height padding between rows
                              wspace=0.38, # Width padding between columns
                              height_ratios=[1.0, 1.0])  # Row 1 taller, Row 2 shorter
        
        # Module display names
        module_display_names = {
            "adsorbate_processor": "Adsorbate Attention",
            "solvent_processor": "Solvent Attention",
            "interaction_attention": "Interaction Attention"
        }
        
        # Row 1: YZ plane attention heatmaps
        ...
        # Row 2: Radial distribution plots
        for col_idx, module_name in enumerate(common_modules):
            avg_attention = sample_averaged_attention[module_name]
            display_name = module_display_names.get(module_name, module_name)
            ...
            # Calculate radial distribution
            d, h, w = avg_attention.shape  # (20, 20, 20)
            center_coords = np.array([d//2, h//2, w//2])  # [10, 10, 10] in voxel indices
            coords = np.array(np.meshgrid(np.arange(d), np.arange(h), np.arange(w), indexing='ij'))
            coords = coords.reshape(3, -1).T
            
            # Calculate distances in voxel indices, then convert to Angstroms
            distances_voxel = np.linalg.norm(coords - center_coords, axis=1)
            voxel_resolution = 0.8  # Å per voxel
            distances = distances_voxel * voxel_resolution  # Convert to Angstroms
            attention_flat = avg_attention.flatten()
            
            # Bin the distances (now in Angstroms)
            max_dist = np.max(distances)  # Should be ~13.86 Å (sqrt(3)*10*0.8)
            bins = np.linspace(0, max_dist, 10)
            bin_centers = (bins[:-1] + bins[1:]) / 2
            ...
            # Plot radial distribution (no title for second row)
            ax.plot(bin_centers, bin_attention, 'bo-', linewidth=2, markersize=8)
            ax.set_xlabel('Distance from center (Å)', fontsize=self.font_size)
            ax.set_ylabel('Average Attention', fontsize=self.font_size)
            ax.tick_params(labelsize=self.font_size)
            ax.grid(False)  # Remove grid lines for clean white background
            ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda x, p: f'{x:.2f}'))
            
            # Set x-axis ticks with interval of 5 Å
            ax.set_xticks([0, 5, 10, 15])
            
            # Set uniform y-axis range for all three radial distribution plots
            ax.set_ylim(0.5, 0.9)
            
            # Adjust aspect ratio for RDF plots to be wider and shorter
            ax.set_box_aspect(1.0)  # Make plots wider (lower value = wider)
        ...
        if save_fig:
            model_info = self.results_filename.replace('.pkl', '').split('-epochs_')[0]
            filename = f"attention_simple_{view_angle.lower()}-{model_info}-{zeolite}-{env}-{pore_type}-{adsorbate}-snap{snapshot}-vox{voxel_id}.png"
            save_path = os.path.join(self.output_dir, filename)
            plt.savefig(save_path, dpi=1000, bbox_inches='tight', facecolor='white')
            print(f"\n✅ Saved simplified {view_angle} plane attention plot: {save_path}")
        ...
        # Print summary
        print(f"\n" + "="*80)
        print("🎨 SIMPLIFIED VISUALIZATION SUMMARY:")
        print("="*80)
        print("📊 Layout: 2 rows × 3 columns")
        print(f"   • Row 1: {view_angle} plane attention heatmaps (adsorbate, solvent, interaction)")
        print("   • Row 2: Radial distribution plots (adsorbate, solvent, interaction)")
        print()
        print(f"🔍 Selected view: {view_angle} plane")
        if view_angle == 'YZ':
            print("   • Best view for visualizing zeolite pore structure")
            print("   • Clearest representation of channel geometry")
        elif view_angle == 'XZ':
            print("   • Side view showing vertical distribution")
            print("   • Good for analyzing layered structures")
        else:  # XY
            print("   • Top-down view showing horizontal distribution")
            print("   • Good for analyzing planar arrangements")
        print()
        print("💡 Hot colors (red/yellow) = High attention | Cool colors (blue) = Low attention")
        print("="*80)
...
if __name__ == "__main__":
    ...
    # analyzer.plot_comprehensive_attention_overlay_simple(
    #     zeolite=zeolite_type,
    #     env=solvent_type,
    #     pore_type=pore_type,
    #     adsorbate=adsorbate,
