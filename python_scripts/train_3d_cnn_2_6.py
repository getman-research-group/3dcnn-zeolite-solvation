...
from model_3d_cnn_2_6 import AttentionCNNTransformer_2_6
...
class CNN3DTrainer:
    ...
    def __init__(self,
        ...
        if not self.job_id:
            ...
            if slurm_job_id:
                # Use output prefix for consistent naming with files
    ...
    def create_model(self, dropout_rate=0.25):
        ...
        if self.model_type == 'cnn':
            ...
            model = AttentionCNNTransformer_2_6(
                in_channels=28, 
        ...
        if os.path.exists(checkpoint_path):
            ...
            if saved_model_type == 'cnn':
                ...
                model = AttentionCNNTransformer_2_6(
                    ...
                    transformer_heads=model_config.get('transformer_heads', 4),  # Updated default
    ...
    def train_single_model(self, X_train, y_train, X_test, y_test, fold_idx, train_idx, test_idx):
        ...
        # 🔥 EMERGENCY FIX: Much stronger gradient clipping to prevent BatchNorm explosion
        # Current gradients: 1.5812 (too high), BatchNorm vars exploding to 1700+
        max_grad_norm = 1.0  # Reduced back to 1.0 to aggressively control training
        ...
        early_stop_patience = 15  # Still much shorter than before
        ...
        for epoch in range(self.epochs):
            ...
            if self.verbose:
                ...
                # 🔥 UPDATED: Get fixed attention temperature for focus monitoring
                temp_info = ""
                if hasattr(model, 'vision_transformer') and model.vision_transformer is not None:
                    for layer in model.vision_transformer.transformer_layers:
                        if hasattr(layer.self_attention, 'temperature'):
                            temp_value = layer.self_attention.temperature  # Now it's a fixed value
                            temp_info = f", Temp={temp_value:.4f}"
                            break  # Only show first layer's temperature
                ...
                print(f"\n  Epoch {epoch + 1:3d}/{self.epochs}: "
                      ...
                      f"LR={old_lr:.6f}{lr_change}{grad_info}{temp_info}, "
                ...
                # 🔥 NEW: Add temperature scaling monitoring
                try:
                    if model is not None:
                        # Check for Vision Transformer temperature scaling
                        for name, module in model.named_modules():
                            if hasattr(module, 'temperature') and hasattr(module, 'multi_head_attention'):
                                temp_value = module.temperature.item() if hasattr(module.temperature, 'item') else module.temperature
                                current_monitoring_data['vision_transformer_temperature'] = temp_value
                                break
                        
                        # 🔥 NEW: Comprehensive Vision Transformer analysis
                        if hasattr(model, 'vision_transformer'):
                            vit = model.vision_transformer
                            
                            # Get detailed attention analysis
                            if hasattr(vit, 'get_attention_summary'):
                                attention_summary = vit.get_attention_summary()
                                if isinstance(attention_summary, dict):
                                    # Add VIT-specific monitoring data
                                    current_monitoring_data['vit_attention_analysis'] = attention_summary
                                    
                                    # Extract key metrics for easy monitoring
                                    if 'attention_layers' in attention_summary:
                                        layers = attention_summary['attention_layers']
                                        if layers:
                                            # Average across all layers
                                            avg_head_diversity = sum(layer.get('head_diversity', 0) for layer in layers.values()) / len(layers)
                                            avg_saturation = sum(layer.get('saturation_ratio', 0) for layer in layers.values()) / len(layers)
                                            avg_entropy = sum(layer.get('attention_entropy', 0) for layer in layers.values()) / len(layers)
                                            
                                            current_monitoring_data['vit_head_diversity'] = avg_head_diversity
                                            current_monitoring_data['vit_saturation_ratio'] = avg_saturation  
                                            current_monitoring_data['vit_attention_entropy'] = avg_entropy
                                    
                                    # Pooling analysis
                                    if 'pooling_analysis' in attention_summary:
                                        pooling = attention_summary['pooling_analysis']
                                        current_monitoring_data['vit_pooling_strategy'] = pooling.get('learned_strategy', 'unknown')
                                        current_monitoring_data['vit_avg_dominance'] = pooling.get('avg_dominance', 0)
                                        current_monitoring_data['vit_max_dominance'] = pooling.get('max_dominance', 0)
                        
                        # Check for gate weights
                        if hasattr(model, 'fusion_gate') and hasattr(model.fusion_gate, 'weight'):
                            gate_weight = torch.norm(model.fusion_gate.weight).item()
                            current_monitoring_data['fusion_gate_weight_norm'] = gate_weight
                except Exception as e:
                    # Silently continue if temperature monitoring fails
                    pass
        ...
        if self.verbose:
            ...
            transformer_weights = [norm for name, norm in final_weight_norms.items() if 'transformer' in name]
            classifier_weights = [norm for name, norm in final_weight_norms.items() if 'classifier' in name]
            ...
            if transformer_weights:
                print(f"      Transformer weights: μ={np.mean(transformer_weights):.3f}, σ={np.std(transformer_weights):.3f}, range=[{min(transformer_weights):.3f}, {max(transformer_weights):.3f}]")
            if classifier_weights:
                print(f"      Classifier weights: μ={np.mean(classifier_weights):.3f}, σ={np.std(classifier_weights):.3f}, range=[{min(classifier_weights):.3f}, {max(classifier_weights):.3f}]")
    ...
    def _format_bn_layer_name(self, layer_name):
        ...
        if '.' in layer_name:
            ...
            elif 'vision_transformer' in layer_name or 'transformer' in layer_name:
                return f"vit.{parts[-1]}"
            else:
    ...
    def _print_monitoring_summary(self, monitoring_data, epoch, batch_idx, model=None):
        ...
        if 'current_weight_norms' in monitoring_data:
            ...
            for layer_name, norm in current_weight_norms.items():
                if any(keyword in layer_name for keyword in ['adsorbate_branch', 'solvent_branch', 'interaction']):
                    ...
                    component_groups['Transformer'].append((layer_name, norm))
                elif 'classifier' in layer_name:
                    component_groups['Classifier'].append((layer_name, norm))
                else:
        ...
        # Expected activation keys from the new analyze_activations method
        activation_keys = [
            'adsorbate_branch', 'solvent_branch', 'combined_features', 'interaction_features',
            'layer1', 'layer2', 'layer3', 'layer4', 'adaptive_pool', 'vision_transformer', 'output'
        ]
        ...
        for key in activation_keys:
            ...
            if f'{key}_mean' in monitoring_data:
                ...
                if f'{key}_abs_mean' in monitoring_data and key in ['adsorbate_branch', 'solvent_branch', 'vision_transformer', 'output']:
        ...
        if model is not None:
            try:
                ...
                # Legacy feature analysis (for models with input_attention)
                elif hasattr(model, 'input_attention') and hasattr(model.input_attention, 'last_attention_weights'):
                    print(f"      🔍 Legacy Input Attention Analysis:")
        ...
        # Traditional layer statistics from monitoring data
        layer_stats = {}
        for key, value in monitoring_data.items():
            if any(layer in key for layer in ['layer1', 'layer2', 'layer3', 'layer4', 'initial_conv', 'adsorbate_processor', 'solvent_processor']):
            ...
            try:
                ...
                if hasattr(model, 'adsorbate_processor') and hasattr(model, 'solvent_processor'):
                    ...
                    if hasattr(model, 'get_processor_statistics'):
                        ...
                        if processor_stats:
                            ads_stats = processor_stats.get('adsorbate', {})
                            sol_stats = processor_stats.get('solvent', {})
                            
                            print(f"        Adsorbate processor: {ads_stats.get('avg_activation', 'N/A'):.3f} avg activation")
                            print(f"        Solvent processor: {sol_stats.get('avg_activation', 'N/A'):.3f} avg activation")
                            
                            if 'channel_utilization' in ads_stats:
                                print(f"        Adsorbate channel utilization: {ads_stats['channel_utilization']:.3f}")
                            if 'channel_utilization' in sol_stats:
                                print(f"        Solvent channel utilization: {sol_stats['channel_utilization']:.3f}")
                    else:
                        print(f"        Processors active but statistics not available")
                
                # Check for fusion layer
                if hasattr(model, 'interaction_conv') or hasattr(model, 'fusion_layer'):
                    print(f"      🔗 Fusion Layer detected:")
                    fusion_layer = getattr(model, 'interaction_conv', None) or getattr(model, 'fusion_layer', None)
                    if fusion_layer and hasattr(fusion_layer, 'weight'):
                        fusion_weight_norm = torch.norm(fusion_layer.weight).item()
                        print(f"        Fusion weight norm: {fusion_weight_norm:.3f}")
            except Exception as e:
        ...
        if model is not None and hasattr(model, 'vision_transformer'):
            try:
                # Check transformer layers for relative position encoding
                for i, layer in enumerate(model.vision_transformer.transformer_layers):
                    if hasattr(layer, 'self_attention') and hasattr(layer.self_attention, 'get_relative_position_stats'):
                        rel_pos_stats = layer.self_attention.get_relative_position_stats()
                        if rel_pos_stats['enabled']:
                            print(f"      - Relative Position Encoding (Layer {i+1}):")
                            print(f"        bias_range: [{rel_pos_stats['min_bias']:.4f}, {rel_pos_stats['max_bias']:.4f}]")
                            print(f"        head_diversity: {rel_pos_stats['head_diversity']:.4f}")
                            print(f"        contribution: {rel_pos_stats['bias_contribution']}")
                            attention_analysis_printed = True
            except Exception as e:
                print(f"      Relative position encoding analysis error: {e}")
        ...
        # - Vision Transformer Self-Attention Analysis
        vit_self_attention_found = False
        
        # Check for VIT attention analysis data (primary source from actual training)
        vit_data_sources = [
            ('vit_attention_analysis', monitoring_data.get('vit_attention_analysis')),
            ('Attention Analysis', monitoring_data.get('Attention Analysis')),
            ('attention_info', monitoring_data.get('attention_info', {}).get('vision_transformer'))
        ]
        
        for source_name, vit_data in vit_data_sources:
            if vit_data and isinstance(vit_data, dict) and 'attention_layers' in vit_data:
                try:
                    attention_layers = vit_data['attention_layers']
                    print(f"      - Vision Transformer Self-Attention:")
                    
                    for layer_name, layer_data in attention_layers.items():
                        if isinstance(layer_data, dict):
                            # Extract VIT-specific metrics
                            mean_attn = layer_data.get('mean_attention', 'N/A')
                            entropy = layer_data.get('attention_entropy', 'N/A')
                            head_div = layer_data.get('head_diversity', 'N/A')
                            saturation = layer_data.get('saturation_ratio', 'N/A')
                            attention_health = layer_data.get('attention_health', 'unknown')
                            
                            # Format numerical values
                            mean_str = f"{mean_attn:.6f}" if isinstance(mean_attn, (int, float)) else mean_attn
                            entropy_str = f"{entropy:.3f}" if isinstance(entropy, (int, float)) else entropy
                            head_div_str = f"{head_div:.4f}" if isinstance(head_div, (int, float)) else head_div
                            saturation_str = f"{saturation:.4f}" if isinstance(saturation, (int, float)) else saturation
                            
                            print(f"        {layer_name}: mean_attn={mean_str}, entropy={entropy_str}, head_div={head_div_str}, sat={saturation_str}")
                            
                            # VIT-specific interpretation
                            if head_div != 'N/A' and isinstance(head_div, (int, float)):
                                if head_div < 0.3:
                                    print(f"        └─ ⚠️ Low head diversity ({head_div:.4f}) - VIT heads may be redundant")
                                else:
                                    print(f"        └─ ✅ Good head diversity ({head_div:.4f}) - VIT heads learning different patterns")
                            
                            # Health assessment
                            if attention_health == 'healthy':
                                print(f"        └─ ✅ Attention health: {attention_health}")
                            elif attention_health == 'needs_attention':
                                print(f"        └─ ⚠️ Attention health: {attention_health}")
                    
                    vit_self_attention_found = True
                    attention_analysis_printed = True
                    break  # Found data, no need to check other sources
                    
                except Exception as e:
                    print(f"      VIT self-attention analysis error ({source_name}): {e}")
                    continue
        
        if not vit_self_attention_found:
            print(f"      - No Vision Transformer self-attention data available")
        
        # - VISION TRANSFORMER DETAILED ANALYSIS:
        print(f"\n    🤖 VISION TRANSFORMER DETAILED ANALYSIS:")
        
        # Check for comprehensive VIT analysis data
        if 'vit_attention_analysis' in monitoring_data:
            vit_analysis = monitoring_data['vit_attention_analysis']
            
            # Overall health assessment
            if 'overall_health' in vit_analysis:
                health = vit_analysis['overall_health']
                status = health.get('status', 'unknown')
                
                if status == 'healthy':
                    print(f"      🟢 Overall Status: HEALTHY Vision Transformer")
            ...
            if 'attention_layers' in vit_analysis:
                layers = vit_analysis['attention_layers']
                print(f"      📊 Layer-by-layer Analysis:")
                
                for layer_name, layer_data in layers.items():
                    head_diversity = layer_data.get('head_diversity', 0)
                    saturation = layer_data.get('saturation_ratio', 0)
                    attention_health = layer_data.get('attention_health', 'unknown')
                    
                    # Health indicator
                    health_icon = "✅" if attention_health == 'healthy' else "⚠️"
                    
                    print(f"        {layer_name}: {health_icon}")
                    print(f"          Head Diversity: {head_diversity:.3f} {'(Good)' if head_diversity > 0.3 else '(Low - needs optimization)'}")
                    print(f"          Saturation: {saturation:.1%} {'(Healthy)' if 0.02 < saturation < 0.4 else '(Issue)' if saturation > 0.4 else '(Weak)'}")
                    print(f"          Entropy: {layer_data.get('attention_entropy', 0):.2f}")
                    
                    # 🔧 DEBUG: Show threshold info for verification
                    if 'uniform_threshold' in layer_data:
                        print(f"          Debug - Uniform Threshold: {layer_data['uniform_threshold']:.4f}")
                    if 'strong_threshold' in layer_data:
                        print(f"          Debug - Strong Threshold: {layer_data['strong_threshold']:.4f}")
                    if 'attention_range' in layer_data:
                        print(f"          Debug - Attention Range: {layer_data['attention_range']}")
                    
                    # Optimization flags
                    if layer_data.get('needs_temperature_increase', False):
                        print(f"          🌡️ Recommendation: Increase temperature (high saturation)")
                    if layer_data.get('needs_diversity_boost', False):
                        print(f"          🎯 Recommendation: Boost head diversity (add dropout/reduce capacity)")
            
            # Pooling strategy analysis
            if 'pooling_analysis' in vit_analysis:
                pooling = vit_analysis['pooling_analysis']
                strategy = pooling.get('learned_strategy', 'unknown')
                avg_dom = pooling.get('avg_dominance', 0)
                max_dom = pooling.get('max_dominance', 0)
                avg_weight = pooling.get('avg_pooling_weight', 0)
                max_weight = pooling.get('max_pooling_weight', 0)
                
                print(f"      🎯 Pooling Strategy Analysis:")
                print(f"        Strategy: {strategy}")
                print(f"        Pooling weights: avg={avg_weight:.3f}, max={max_weight:.3f}")
                print(f"        Dominance ratios: avg={avg_dom:.3f}, max={max_dom:.3f}")
                
                # Enhanced interpretation with weight analysis
                total_weight = avg_weight + max_weight
                if total_weight > 0:
                    avg_percentage = (avg_weight / total_weight) * 100
                    max_percentage = (max_weight / total_weight) * 100
                    print(f"        Weight distribution: avg={avg_percentage:.1f}%, max={max_percentage:.1f}%")
                
                # Interpretation
                if strategy == 'avg_dominant':
                    print(f"        └─ 💡 Model prefers smooth, overall molecular features")
                elif strategy == 'max_dominant':
                    print(f"        └─ 💡 Model focuses on peak interaction strengths")
                else:
                    print(f"        └─ 💡 Balanced approach to feature aggregation")
        
        # Look for individual VIT metrics in monitoring_data
        transformer_data = {}
        for key, value in monitoring_data.items():
            if ('vision_transformer' in key.lower() or 'vit_' in key.lower()) and key not in ['Attention Analysis', 'vit_attention_analysis']:
                transformer_data[key] = value
        
        if transformer_data:
            print(f"      📈 Real-time VIT Metrics:")
            
            # 🔥 NEW: Enhanced saturation monitoring with optimization tracking
            for key, value in transformer_data.items():
                if key == 'vit_head_diversity':
                    print(f"        Head Diversity: {value:.3f} {'✅ Good' if value > 0.3 else '⚠️ Low - needs optimization'}")
                elif key == 'vit_saturation_ratio':
                    print(f"        Saturation Ratio: {value:.1%} {'✅ Healthy' if 0.02 < value < 0.4 else '🚨 HIGH' if value > 0.4 else '⚠️ Weak'}")
                    if value > 0.4:
                        print(f"        └─ 💡 Recommendation: Increase temperature scaling or use softer activations")
                    elif value < 0.02:
                        print(f"        └─ 💡 Recommendation: Decrease temperature scaling or check input preprocessing")
                elif key == 'vit_attention_entropy':
                    print(f"        Attention Entropy: {value:.2f} {'✅ Good diversity' if 1.0 < value < 3.0 else '⚠️ Check attention patterns'}")
                elif key == 'vit_pooling_strategy':
                    print(f"        Pooling Strategy: {value}")
                elif key == 'vit_avg_dominance':
                    print(f"        Avg Dominance: {value:.3f}")
                elif key == 'vit_max_dominance':  
                    print(f"        Max Dominance: {value:.3f}")
                elif key == 'vision_transformer_temperature':
                    print(f"        Temperature: {value:.3f} {'✅ Optimized' if value >= 0.08 else '⚠️ May cause over-smoothing'}")  # Updated threshold
                elif key.startswith('vit_'):
                    if isinstance(value, (int, float)):
                        # 🔧 NEW: Handle debug threshold information
                        if 'threshold' in key:
                            print(f"        {key.replace('vit_', '').replace('_', ' ').title()}: {value:.4f}")
                        else:
                            print(f"        {key.replace('vit_', '').replace('_', ' ').title()}: {value:.3f}")
                    else:
                        # Handle attention_range and other string debug info
                        print(f"        {key.replace('vit_', '').replace('_', ' ').title()}: {value}")
                else:
                    if isinstance(value, (int, float)):
                        print(f"        {key}: {value:.3f}")
                    elif key == 'Attention Analysis' and isinstance(value, dict):
                        # Use the dedicated function to format attention analysis nicely (brief version for training)
                        self._print_formatted_attention_analysis(value, indent="        ", brief=True)
                    else:
                        # Format complex dictionaries
                        if isinstance(value, dict) and len(str(value)) > 100:
                            ...
                            for k, v in value.items():
                                if isinstance(v, float):
                                    ...
                                    print(f"          {k}: (nested dict with {len(v)} items)")
                                else:
                                    print(f"          {k}: {v}")
                        else:
                            # Special handling for 'Attention Analysis' to prevent raw dict output
                            if key == 'Attention Analysis':
                                # Use the dedicated function instead of raw print (brief version for training)
                                self._print_formatted_attention_analysis(value, indent="        ", brief=True)
                            else:
                                print(f"        {key}: {value}")
            
            print(f"        └─ 📍 Architecture: Relative position encoding (no center bias)")
            print(f"        └─ 🔧 Applied Optimizations: Fixed saturation calculation, enhanced thresholds")
        
        # Fallback: Look for any transformer-related data if no VIT-specific data found
        elif any('transformer' in key.lower() for key in monitoring_data.keys()):
            print(f"      📊 Basic Transformer Metrics:")
            for key, value in monitoring_data.items():
                if 'transformer' in key.lower():
                    if isinstance(value, (int, float)):
                        print(f"        {key}: {value:.3f}")
                    else:
                        # Format complex dictionaries
                        if isinstance(value, dict) and len(str(value)) > 100:
                            ...
                            # Special handling for 'Attention Analysis' with dedicated formatting
                            if key == 'Attention Analysis' and isinstance(value, dict):
                                # Use the dedicated function for clean formatting (brief version for training)
                                self._print_formatted_attention_analysis(value, indent="        ", brief=True)
                            elif key != 'Attention Analysis':
                                # Only print non-Attention Analysis keys
                                print(f"        {key}: {value}")
        else:
            print(f"      ℹ️ No Vision Transformer monitoring data available this epoch")
                        ...
                        if total_bn_layers > 4:
                            ...
                            for layer_name in bn_stats.keys():
                                if 'adsorbate' in layer_name:
                                    ...
                                    layer_types['transformer'] = layer_types.get('transformer', 0) + 1
                                else:
    ...
    def _print_formatted_attention_analysis(self, attention_dict, indent="        ", brief=True):
        ...
        if brief:
            ...
            if 'cross_attention' in attention_dict:
                cross_attn = attention_dict['cross_attention']
                cross_count = len(cross_attn)
                print(f"{indent}  🔗 Cross-Attention: {cross_count} layer(s) analyzed")
...
def parse_arguments():
    ...
    # Output file prefix argument - hardcoded for train_3d_cnn_2_1.py
    parser.add_argument('--output-prefix', type=str, default='model_2_1',
