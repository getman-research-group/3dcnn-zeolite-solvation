...
class MultiHeadAttention3D(nn.Module):
    ...
    def __init__(self, d_model, num_heads, dropout=0.1, max_len=8, use_relative_pos=True):
        ...
        # 🔥 SIMPLIFIED: Remove learnable temperature - let model handle it naturally
        # Learnable temperature for dynamic attention scaling
        self.temperature = nn.Parameter(torch.ones(1) * 0.1)  # More conservative temperature
...
class VisionTransformer3D(nn.Module):
    ...
    def get_attention_summary(self):
        ...
        if hasattr(self, 'pooling_weights'):
            ...
            avg_ratio = weights[0] / (total_weight + 1e-8)
            max_ratio = weights[1] / (total_weight + 1e-8)
            ...
            pooling_analysis = {
                ...
                'learned_strategy': 'avg_dominant' if avg_ratio > 0.6 else 'max_dominant' if max_ratio > 0.6 else 'balanced'
            }
...
class AttentionCNNTransformer_2_3(nn.Module):
    """
    ...
    def __init__(self, in_channels=28,  # Changed default from 15 to 28 for type_2 format
        ...
        super(AttentionCNNTransformer_2_3, self).__init__()
    ...
    def _initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv3d):
                # Optimized initialization for better gradient flow
                nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu', a=0.1)
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
            elif isinstance(m, nn.BatchNorm3d):
                # Better BatchNorm initialization to address variance issues
                nn.init.constant_(m.weight, 0.8)  # Start slightly below 1 for stability
                nn.init.constant_(m.bias, 0)
                # Set momentum for more stable running statistics
                m.momentum = 0.05  # Reduced from default 0.1 for more stable statistics
            elif isinstance(m, nn.Linear):
                # Balanced Xavier initialization
                nn.init.xavier_normal_(m.weight, gain=0.5)  # Increased from 0.3 for better capacity
                if m.bias is not None:
                    nn.init.constant_(m.bias, 0)
