...
from model_3d_cnn_2_2 import AttentionCNNTransformer_2_2
...
class CNN3DTrainer:
    ...
    def create_model(self, dropout_rate=0.25):
        ...
        if self.model_type == 'cnn':
            ...
            model = AttentionCNNTransformer_2_2(
                in_channels=28, 
    ...
    def _print_monitoring_summary(self, monitoring_data, epoch, batch_idx, model=None):
        ...
        # New: Relative Position Encoding Analysis
                    ...
                    if self_attention_layers:
                        ...
                        for layer_name, layer_data in self_attention_layers.items():
                            if isinstance(layer_data, dict):
                                ...
                                # 📊 NEW: Reflects relative position encoding
                                
                                # 📊 Precision optimization for better readability
                                # Format all numerical values with appropriate precision
                                strength_str = f"{strength:.6f}" if isinstance(strength, (int, float)) else strength
                                spatial_var_str = f"{spatial_var:.6f}" if isinstance(spatial_var, (int, float)) else spatial_var
                                ...
                                print(f"        {layer_display}.self_attention: strength={strength_str}, spatial_var={spatial_var_str}, entropy={entropy_str}, head_div={feature_div_str}")
                                ...
                                # 💡 Updated interpretation hints for relative position encoding
                                if feature_div != 'N/A' and isinstance(feature_div, (int, float)):
                                    if feature_div < 0.0005:
                                        print(f"        └─ ⚠️ Very low head diversity ({feature_div:.4f}) - heads may be redundant")
                                    elif feature_div < 0.001:
                                        print(f"        └─ ⚠️ Low head diversity ({feature_div:.4f}) - consider fewer heads")
                                    elif feature_div > 0.001:
                                        print(f"        └─ ✅ Good head diversity ({feature_div:.4f}) - heads learning different patterns")
                                        print(f"        └─ 📍 Using relative position encoding (no center bias)")
                                else:
        ...
        if 'vit_attention_analysis' in monitoring_data:
            ...
            if 'attention_layers' in vit_analysis:
                ...
                for layer_name, layer_data in layers.items():
                    ...
                    print(f"          Saturation: {saturation:.1%} {'(Healthy)' if 0.05 < saturation < 0.3 else '(Issue)' if saturation > 0.3 else '(Weak)'}")
            ...
            if 'pooling_analysis' in vit_analysis:
                ...
                print(f"        Average pooling dominance: {avg_dom:.2f}")
                print(f"        Max pooling dominance: {max_dom:.2f}")
        ...
        transformer_data = {}
        for key, value in monitoring_data.items():
            if 'vision_transformer' in key.lower() or 'vit_' in key.lower():
        ...
        if transformer_data:
            ...
            for key, value in transformer_data.items():
                if key == 'vit_head_diversity':
                    ...
                    print(f"        Saturation Ratio: {value:.1%} {'✅ Healthy' if 0.05 < value < 0.3 else '🚨 HIGH' if value > 0.3 else '⚠️ Weak'}")
                    if value > 0.3:
                    ...
                    print(f"        Temperature: {value:.3f} {'✅ Optimized' if value >= 0.2 else '⚠️ May cause saturation'}")
            ...
            print(f"        └─ � Applied Optimizations: GELU+Tanh activation limits, increased dropout")
        ...
        # 🔥 NEW: Optimization Status Summary
        print(f"\n    🎯 OPTIMIZATION STATUS SUMMARY:")
        print(f"      🔧 Applied Anti-Saturation Optimizations:")
        print(f"        • Vision Transformer: GELU+Tanh activation limits")
        print(f"        • Temperature Scaling: Increased from 0.1 → 0.2")
        print(f"        • Adsorbate Branch: Enhanced processing layers")
        print(f"        • Feed-Forward: Increased dropout (0.1 → 0.15)")
        print(f"        • Classifier: Tanh output limiting")
        print(f"        • Gate Weights: Reduced from 0.1 → 0.05")
        
        # 🔥 NEW: Real-time optimization parameter monitoring
        if 'vision_transformer_temperature' in monitoring_data:
            temp_value = monitoring_data['vision_transformer_temperature']
            print(f"      🌡️ Current Temperature Scaling: {temp_value:.3f}")
            if temp_value >= 0.2:
                print(f"        └─ ✅ Optimized temperature (≥0.2) - reducing attention intensity")
            else:
                print(f"        └─ ⚠️ Low temperature ({temp_value:.3f}) - may cause saturation")
        
        if 'fusion_gate_weight_norm' in monitoring_data:
            gate_norm = monitoring_data['fusion_gate_weight_norm']
            print(f"      🚪 Current Gate Weight Norm: {gate_norm:.3f}")
            if gate_norm <= 0.05:
                print(f"        └─ ✅ Optimized gate weights (≤0.05) - controlled fusion")
            else:
                print(f"        └─ ⚠️ High gate weights ({gate_norm:.3f}) - may dominate features")
        
        print(f"      📊 Target Monitoring Thresholds:")
        print(f"        • Vision Transformer Saturation: < 30% (optimal: 15-30%)")
        print(f"        • Adsorbate Branch Importance: 25-35% (was 17.1%)")
        print(f"        • Temperature Effect: Reduced attention intensity")
        print(f"        • Overfitting Ratio: < 2.0 (was 2.74)")
