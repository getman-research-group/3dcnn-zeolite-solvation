...
with open('traj_intE.txt','r') as traj_intE:
    ...
    for line in lines:
        if line.startswith('intE01/aq '):
            index_intE = lines.index(line)
        if line.startswith('00_intE_zeo'):
            index_zeo_intE = lines.index(line)
            # print(index_intE)
traj_intE.close()

calc_intE=[]
for line in lines[index_intE:index_intE + 40]:
    if line != []:
...
zeo_ref_g = float(lines[index_zeo_intE].split()[1])
# print(zeo_ref_g)
