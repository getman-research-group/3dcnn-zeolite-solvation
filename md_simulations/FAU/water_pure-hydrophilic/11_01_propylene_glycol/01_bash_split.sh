...
cutoff=7
adsorbate_file=11_01_propylene_glycol
...
cp /zfs/curium/xiutinc/zeolite/FAU/intE_dft_one_unitcell/03_01_1_3_propanediol/{02_bash_g_sp.sh,03_bash_traj_intE_calc.sh,calc_intE.py,traj_2_intE.py,zeolite_ad_g.py} ./
...
######################## 1. split trajectory to 10 xyz file ########################
sed -i 's/\r$//' *.lammpstrj
...
######################## 2.dft simulation for each frame in aqueous phase #############################
### for each traj make xyz_zeo_ad_aq and xyz_zeo_aq in aqueous phase
